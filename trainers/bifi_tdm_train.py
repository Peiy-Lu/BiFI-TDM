...
def default_train(train_loader,model,optimizer,writer,iter_counter):
    ...
    for i, (inp,_) in enumerate(train_loader):
        ...
        iter_counter += 1
        inp = inp.cuda()
        log_prediction, s, fc1, fc2, M = model(inp)
        ...
        loss = class_loss + aux_loss + args.gamma_loss*anchor_loss
...
def meta_train(train_loader, model, optimizer, writer, iter_counter):
    ...
    for i, (inp, label) in enumerate(train_loader):
        ...
        inp = inp.cuda()
        label = label.cuda()
        log_prediction, s, fc1, fc2, M = model(inp=inp)
        ...
        loss = cls_loss + args.gamma_loss*anchor_loss
