...
import numpy as np
from .backbones import Conv_4_FME, Conv_4_MFSE, ResNet_FME, ResNet_MFSE
from .TDM import TDM
...
class BiFI_TDM(nn.Module):
    ...
    def __init__(self, way=None, shots=None, resnet=False, is_pretraining=False, num_cat=None, args=None):
        ...
        if resnet:
            num_channel = 640
            self.feature_extractor_1 = ResNet_FME.resnet12(args=args)
            self.feature_extractor_2 = ResNet_MFSE.resnet12(args=args)
            self.Woodbury = False
        else:
            num_channel = 64
            self.feature_extractor_1 = Conv_4_FME.BackBone(num_channel, args=args)
            self.feature_extractor_2 = Conv_4_MFSE.BackBone(num_channel, args=args)
            self.Woodbury = True
    ...
    def get_neg_l2_dist(self, inp, way, shot, query_shot, return_support=False, Woodbury=True):
        ...
        if return_support:
            return neg_l2_dist, (support_1,support_2), self.fc1(F.relu(feature_map_1.mean(dim=1))), self.fc2(F.relu(feature_map_2.mean(dim=1)))
        else:
            return neg_l2_dist
    ...
    def forward(self, inp):
        ...
        neg_l2_dist, s, fc1, fc2 = self.get_neg_l2_dist(inp=inp,
                                                    way=self.way,
                                                    shot=self.shots[0],
        ...
        return log_prediction, s, fc1, fc2, self.M
